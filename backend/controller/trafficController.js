...
const { fetchRoutesFromTomTom, fetchTrafficIncidents, geocodePlace } = require('../services/tomTomService');
const { fetchWeather } = require('../services/weatherService');
const { predictTrafficDelay, detectAnomaly } = require('../services/aiService');

const getRoutes = async (req, res, next) => {
  try {
    const { startPlace, endPlace, cityLat, cityLng } = req.query; // Changed to req.query for GET
    ...
    const formattedRoutes = await Promise.all(
      routeData.map(async (route) => {
        ...
        const predictedDelay = await predictTrafficDelay(
          ...
          strikes,
          otherEvents,
          startPlace, // Added for app.py compatibility
          endPlace   // Added for app.py compatibility
        );
