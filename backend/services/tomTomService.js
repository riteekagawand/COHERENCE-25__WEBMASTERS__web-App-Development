...
const fetchRoutesFromTomTom = async (start, end) => {
...
const fetchTrafficIncidents = async (boundingBox) => {
  try {
    const response = await axios.get(
...
const geocodePlace = async (placeName, cityLat, cityLng) => {
...
const fetchUtilityLocations = async (category, lat, lng) => {
  try {
    const response = await axios.get(
